...
class FeatureSelector:
    ...
    def identify_zero_importance(
        ...
        if early_stopping and eval_metric is None:
            raise ValueError(
                """eval metric must be provided with early stopping. Examples include "auc" for classification,
        ...
        for i in range(n_iterations):
            if task == "classification":
                ...
                raise ValueError(
                    'Task must be either "classification", "regression", or "quantile"'
                )
        ...
        postive_features_sum = (
            feature_importances["importance"] * (feature_importances["importance"] >= 0)
        ).sum()
        feature_importances["normalized_importance"] = (
            feature_importances["importance"] / postive_features_sum
        )
