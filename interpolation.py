...
class Interp:
    ...
    def nn(self):
        ...
        for n in grid_raster:
            if self.dt.is_inside_convex_hull(n[0], n[1]) == True:
                try:
                    data = [n[0], n[1], self.dt.interpolate_nn(n[0], n[1])]
                except:
                    data = [n[0], n[1], np.nan]
                # except:
                #     print(f'{n[0]}, {n[1]}')
        ...
        self.nn_p = pd.DataFrame(nn_data, columns=["lat", "lon", "interp_h"])
        nn_h = xr.Dataset.from_dataframe(self.nn_p.set_index(["lat", "lon"]))
        return nn_h
...
def _test():
    ...
    tin_interp(tasman_NP, 1, "ICESAT/tasmania/tasmania_tin.tif")
